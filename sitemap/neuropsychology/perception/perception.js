...
const colourPerception = document.createElement('button');
colourPerception.innerHTML = 'COLOUR PERCEPTION TASK';
colourPerception.className ="pageOptions";
colourPerception.id="colourMatchingTask";
optionsDisplay.appendChild(colourPerception);

colourPerception.addEventListener('click',()=>{
    function link(){
        var a = document.createElement('a');
        a.href="/Users/lordyolo/Desktop/website/sitemap/neuropsychology/perception/ colourPerceptionTask/colourPerception.html";
        a.click()
    };
    windowOpen(container);
    setTimeout(link,1500)})

const audiometer = document.createElement('button');
audiometer.innerHTML = 'AUDIOMETER';
audiometer.className = "pageOptions";
audiometer.id = "audiometer";
optionsDisplay.appendChild(audiometer);

audiometer.addEventListener('click',()=>{
    function link(){
        var a = document.createElement('a');
        a.href="/Users/lordyolo/Desktop/website/sitemap/neuropsychology/perception/audiometer/audiometer.html";
        a.click();
    };
        windowOpen(container);
        setTimeout(link,1500)});
